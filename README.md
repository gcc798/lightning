...
这里的“渐进式”指服务边界随真实业务增长：新需求先归入现有领域，只有形成独立数据所有权和部署需求时才拆为新服务。它不是单体与微服务双运行模式；`native` 当前只保留微服务部署形态。
...
├── AGENTS.md    # 代码代理开发约定
├── CLAUDE.md    # Claude 读取 AGENTS.md 的入口
└── README.md
```
...
- `application/gateway`：唯一对外 HTTP/WS 入口，通过注册中心发现并反向代理领域服务，不拥有业务数据。
- `application/{iam,sys,resource}`：独立领域服务；每个服务在自己的 `internal/` 下拥有 controller、router、DTO、领域代码、数据模型和迁移。
- `application/scheduler`：独立定时任务进程，不提供 HTTP 服务，也不执行数据库迁移。
- `cmd/usermgr`：创建管理员、重置密码的一次性管理工具。
- `internal`：多个进程共享、但禁止当前 Go Module 之外导入的技术设施和 gRPC 契约，不存放领域业务实现。

当前部署拓扑为 `gateway + iam + sys + resource + scheduler`。IAM、SYS、Resource 启动时分别执行自己嵌入二进制的 Goose 版本化迁移；数据库结构不使用 GORM `AutoMigrate`。验证码、微信、短信、邮件和 Scheduler 等运行期模块配置保存在 `s_config`，不写入服务 YAML。
...
- 渐进拆分沿事务、数据所有权和独立部署边界进行，不按 controller 数量拆分，也不提前引入分布式事务。
- `native/application/` 下一级只允许真实进程，不增加共享 API 应用、单体入口或跨服务领域层。
